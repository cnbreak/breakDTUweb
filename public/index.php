<?php
    ...
    $app->get('/update', function () use ($app) {
        ...
        if (!$app->user->checkAccess('uri_dashboard')) {
            ...
            $app->render('break/update.twig', ['action' => $_GET ['action'],'id' => $_GET ['id']]);
        }
    });
    ...
    $app->get('/send', function () use ($app) {
        ...
        if (!$app->user->checkAccess('uri_dashboard')) {
            ...
            $app->render('break/send.twig', ['action' => $_GET ['action'],'id' => $_GET ['id']]);
        }
    });
    //End
