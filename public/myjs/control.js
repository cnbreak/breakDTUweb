...
var oTable = $('#jsonGrid').DataTable({
    ...
    columns: [
        ...
        {title: 'workerID', data: 'deviceWorkerId'},
        {title: 'TCP连接ID', data: 'deviceConnectionId'},
...
$('#jsonGrid tbody').on('click', 'tr', function () {
    ...
    window.location.href='send?action=selectdeviceinfowithid&id='+data.deviceInfoId;
});

//显示工作列表
function tablelist() {
    jQuery.ajaxSetup({cache: false});
    $.post("../public/getdata/control.php",function (valArr, status) {
        if (status != 'success') {
